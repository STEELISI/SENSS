...
def listener(queue,date,type,dataset):
	f = open("../../new_results_"+dataset+"/"+date+"_"+type, 'w')
...
def run_process(ip_16,reference_end_time,date,queue,dataset,send_accuracy_scores):
	#generate_prefix_data(current_prefix,reference_end_time,date,output_folder,dataset,accuracy_scores)
	hf_array,f_array,h_array,ip_blacklist_map=generate_prefix_data(ip_16,reference_end_time,date,"../../Results6",dataset,send_accuracy_scores)
...
def generate_pd(dataset):
	os.system("mkdir ../../new_results_"+dataset)
	...
	#Change this
	file_list=glob.glob("../../google_blacklist_lib/relevant_data/"+dataset+"_ip_data/*")
	#+glob.glob("testing_data/"+dataset+"_testing_data/*")
	temp1=set()
	...
	for file in file_list:
		...
		if "testing_data" in file:
			watcher = pool.apply_async(listener, (q,date,"whitelist",dataset,))
		else:
			watcher = pool.apply_async(listener, (q,date,"blacklist",dataset,))
		...
		for ip_16 in ip_16_set:
			...
			job = pool.apply_async(run_process, (ip_16,date_sum,date,q,dataset,send_accuracy_scores,))
			jobs.append(job)
		for job in jobs:
			job.get()
