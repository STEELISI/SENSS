...
def range_check(s_time,e_time,timestamp):
        ...
        #print "s_time",s_time,"e_time",e_time,"timestamp",timestamp
        if s_time=="unknown":
                if timestamp>=e_time:
        ...
        if timestamp>=s_time and e_time==0:
                delay=0
                return (delay,"on_time")
        #print "Condition not met",s_time,e_time,timestamp
...
def send_mail(email_to,subject,message):
        fromaddr = "rorrykeys@gmail.com"
        toaddr = email_to
        server = smtplib.SMTP('smtp.gmail.com', 587)
        server.starttls()
        server.login("rorrykeys@gmail.com", "")
